...
These are three separate checks: AI response rules, generator rules before writing C code, and Vitis report checks after synthesis. Passing the first two means the point is safe to try. It does not mean that Vitis will accept it or that its score will improve.
...
Example: rank 1 and rank 3 are new, but rank 2 repeats an old pragma plan. FORGE keeps ranks 1 and 3 and asks AI for only one replacement. The temporary replacement rank is mapped back to rank 2. If it still repeats after the third total attempt, local fallback fills only rank 2.

If the first response fails JSON, schema, or pragma parsing, no ranks have been accepted yet, so FORGE retries the whole response. Rank-only repair mainly handles repeats found after a full parse and later failures inside repair mode.
...
Fallback does not predict the best score. It builds conservative points that are safe enough to test in Vitis. It uses allowed pipeline II values `1, 2, 4`, unroll factors `2, 4, 8`, and cyclic or block array partition factors `2, 4, 8`. It first uses plans not found in past history. If new plans are not enough, it may use different old safe plans as verification. If the full finite list is too small, it returns fewer points instead of copying the same plan inside the fallback set.

Search policy is not an interactive question on every run. `explore` is the default. FORGE automatically changes from normal explore to stable explore after two no-improvement batches. The user enters verify mode only with `--exploration-mode verify`.

The database has one richer `experiments` table, but it has no `is_converged`, `repair_slots`, or `is_fallback` columns. Repair data exists only during the AI call. Stable-search state is calculated from saved batch status and scores. Fallback points are stored like normal points; their plans are in `pragma_plan_json`, and the fallback reason is kept in the pragma report.
