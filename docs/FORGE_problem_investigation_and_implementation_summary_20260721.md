...
1. **AI 响应检查**：JSON 格式、directive 名称、目标函数、loop ID、数组、factor 和 II 是否符合规则；
2. **代码生成检查**：生成器在写 C 文件前再次检查同一批目标，防止其他调用路径绕过第一层；
3. **Vitis 结果检查**：`csynth` 后检查 pragma 是否真的匹配并生效。前两层通过只代表“可以安全尝试”，不代表一定能综合成功或提高 score。

### 4.4 定向 refine，而不是整批重试

推荐器按照标准化 pragma signature 判断重复，不比较名称。

当一批已经通过格式和 pragma 合法性检查，但只有部分 rank 与历史计划重复时：
...
- rank 1 是新计划，保存到 `accepted[1]`；
- rank 2 与数据库中的完整 pragma plan 相同，加入 `repair_slots=[2]`；
- rank 3 是新计划，保存到 `accepted[3]`；
- 下一次请求只要求 AI 返回一个替代计划。AI 在这次请求中临时使用 rank 1，FORGE 收到后会把它重新映射回原始 rank 2；
- `repair_context` 同时告诉 AI：rank 1 和 rank 3 已经接受，不允许修改；原 rank 2 为什么被拒绝；
- 如果替代计划再次重复，第三次仍只修 rank 2；第三次之后仍失败，fallback 也只补 rank 2。

需要注意一个当前边界：如果第一次响应在 JSON、schema 或 pragma 合法性解析阶段就报错，解析器还没有得到可保留的 rank，此时会要求 AI 重做整份响应。当前的“保留合法 rank、只修失败 rank”主要用于完整解析后发现的历史重复，以及已经进入 repair 模式后的替代失败。

### 4.5 有界 fallback 和收敛状态

fallback 不是根据预测 score 选出“可能最优”的点。它发生在 AI 三次仍不能提供可用替代计划时，用固定规则生成“能够安全进入 Vitis 测试”的保守点，实际好坏仍由后续 Vitis 数据决定。

当前 fallback 的候选规则为：

- 对允许 pipeline 的 loop，尝试 `II = 1, 2, 4`；
- 对允许 unroll 的 loop，尝试 `factor = 2, 4, 8`；
- 对已识别的数组参数，尝试 `cyclic/block ARRAY_PARTITION`，factor 为 `2, 4, 8`；
- 通常组合一个 loop pragma 和一个 memory pragma；如果没有合适数组，则使用保守的 `INLINE off` 作为层级方案；
- 如果没有合法 loop，只探索 memory pragma；两者都没有时，只生成层级验证点。

FORGE 先排除本批已经接受的 plan，并优先选择数据库中从未出现过的 pragma signature。如果新组合不够，它可以使用彼此不同但历史上运行过的安全计划作为 verification；如果整个有限候选集合仍少于缺失 rank 数量，就返回较少的点，不会在同一个 fallback 集合中复制完全相同的计划。

fallback 仍会经过生成器检查和 Vitis 的 pragma-effectiveness 检查。因此“local safe”表示语法和静态目标较保守，不表示它一定有效，更不表示 score 一定高。
...
这里的 search policy 不是每次在终端询问用户。默认参数是 `--exploration-mode explore`：FORGE 在 explore 内部根据数据库结果自动使用正常探索或收敛后的稳定探索。只有用户明确传入 `--exploration-mode verify` 时，才进入允许历史计划复测的 verify 模式。

### 4.5.1 数据库是否为 refine、fallback 和收敛增加字段

数据库确实被改成了统一的 `experiments` 表，并包含 `evaluation_context_key`、`experiment_set`、`batch_number`、原始/生成源码、`pragma_plan_json`、`status`、`metrics_json`、runtime、power、energy、LUT、score 和 error 等字段。

但本次没有增加 `is_converged`、`repair_slots` 或 `is_fallback` 这类专用列：

- `repair_slots` 和 `repair_context` 只在一次 AI 推荐过程的内存中使用，不写数据库；
- 收敛状态由相同 `evaluation_context_key` 下各 `experiment_set` 的 `status` 和 `efficiency_score` 每次动态计算；
- fallback 生成的点与普通 design point 一样保存，完整 pragma 在 `pragma_plan_json` 中，名称通常带有 `local_safe`；fallback 原因保存在本次 pragma report 中，而不是单独的数据库列。
